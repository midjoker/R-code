...
#                       ifelse(data$B_VS_C_Ratio >= 1.2 ,'Up','Down'),'No')
  data$signifcant = ifelse(data$`2-A_VS_2-S_Pvalue` < 0.05 & abs(log2(data$`2-A_VS_2-S_FC`)) >= log2(2),
                         ifelse(log2(data$`2-A_VS_2-S_FC`)>= log2(2),'Up','Down'),'No')
...
str(data)
 p <- ggplot( data = data, 
              aes(x = log2(`2-A_VS_2-S_FC`), 
                  y = -log10(`2-A_VS_2-S_Pvalue`), 
                 colour=signifcant)) +#                 label = Name
         geom_point(alpha=0.5, size=2) +
         scale_color_manual(values=c("green", "grey","red"))+
         xlim(c(-10, 10)) +ylim(c(0,10))+geom_vline(xintercept=c(-log(2,2),log(2,2)),lty=4,col="grey",lwd=0.8) +

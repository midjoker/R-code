...
Org = "human"    #修改物种 主要为人(human)小鼠(mouse)大鼠(rat)三个物种的，其他的根据实际物种情况进行修???
#OrgDB = "org.Ss.eg.db"
...
name <- "top150"  #设置最后生成的GOKEGG文件名称
workbook <- "三个样本top150交集-14个miRNA靶基因.xlsx"  #需要做GOKEGG的数据框，默认读取第一个sheet
data <- read.xlsx("三个样本top50交集-14个miRNA靶基因.xlsx",1)
#View(data)
...
barplot(go_BP, title = "Gene Ontology Biological Process",
        showCategory = 50, cex.names = 0.1, font.size = 9)
